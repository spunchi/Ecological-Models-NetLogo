...
It is assumed that two species cannot coexist in the same environment (Gauss 1932). The competition among two species out compete one of the species and only one species survive in the long run. If two species are equally competitive then stochastic drift determines the species abundances, the stochastic drift caused community to reach monodominance. However, we have seen two species managed to coexist in the same environment. It is assumed that ecological processes such as conspecific negative density dependence (CNND) can maintain the species richness at local spatial scales (_i.e._ tree neighborhood). This model illustrates the CNND (intraspecific competition) in sessile organisms (_e.g._ trees).

## HOW IT WORKS

The model has two species (sp<sub>1</sub> and sp<sub>2</sub>). The initial abundance of the species are _N_<sub>1</sub> and _N_<sub>2</sub>. The community has a size _J_. Each patch contains maximum one individual. Initially, community is saturated.Therefore, initial abundance of the species-2 is, _N_<sub>2</sub> = _J_ - _N_<sub>1</sub>. Two species placed in the community randomly at the beginning. Each time a tree is selected (a focal tree). If the number of conspecifics around the focal tree in its neighborhood (_i.e._ surrounding eight patches) exceeds a certain number (CND-sp) then the focal tree dies due to intense CNND, otherwise focal tree is survived. This process creates empty patches in the community. These empty patches will be filled by the individuals from one of the species. However, the filling process should not violate the above rule that we mentioned (_i.e._ The number of conspecifics, in the surrounding eight patches, should not exceeds the threshold value. If it exceeds the threshold value vacant site cannot be filled and should remained empty).  
...
2. **Initial abundance of species:** 1 (_N_<sub>1</sub>). CND-sp<sub>1</sub> is the maximum number of conspecifics that can be allowed in the eight neigboring patches for a stable community. For example if CND-sp1 is equal to 5 then focal species-1 has 6, 7, or 8 conspecific neighbors then the focal species-1 has to die.
 
3. **Conspecific density on focal species-1:** (CND-sp<sub>1</sub>).

4. **Conspecific density on focal species-2:** (CND-sp<sub>2</sub>). 
...
The model is extended by incorperating stochastic death process. Certain number of trees die (_K_) randomly. Maximum number of deaths per time step is equal to _J_ - 1. When _K_ = J the model becomes a non-overlapping genrations model.
...
* Sareena, B. & Punchi-Manage, R. (2023j).  NetLogo Intraspecific Competition Model.
https://modelingcommons.org/browse/one_model/7274#model_tabs_browse_info
