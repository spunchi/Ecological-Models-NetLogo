...
"" ""
PENS
"pen-0" 1.0 0 -7500403 true "" "plot speciation"
...
The Rosindell _et al_. (2010) algorithm-2 is based on the concept of coalescence, instead of simulating the system forwards in time until it reaches equilibrium, He simulated it backwards in time following the ancestry of each individual in the sample back to the speciation event that gives the species identity of this individual. In algorithm-2 he considered the protracted speciation where _τ_ = τ. He took a sample of size _J_ from the metacommunity of size _J_<sub>M</sub> and traced _K_ lineages backwards in time where initially _K_ = _J_.
...
1. Input _θ_ and _J_ and _τ_.

2. Define a vector _V_ of length _J_ with _V_<sub>_i_</sub> = 1 for _i_ = 1, 2, . . . _J_ and define _K_ = _J_, the number of lineages in the vector.

3. Define _t_ = 0 the total number of units of coalescence time that have passed.

4. Pick a random number 0 ≤ _r_ ≤ 1 (to choose how many generations until the next coalescence event).

5. Set t = t − ( ln(r) / [K(K−1)] ).

6. If _τ_ > t, pick two random entries _V_<sub>i</sub> and _V_<sub>j</sub>  from the vector _V_ (where _j_ not equal to _i_), set _K_ = _K_ - 1 and _V_<sub>_i_</sub> = _V_<sub>_i_</sub> + _V_<sub>_j_</sub>. Then remove _V_<sub>_j_</sub> from the vector _V_ (then go to 3).

7. If _τ_ _≤ t_, define an empty vector _W_ which will contains a list of species abundances when the algorithm terminates.

8. Pick a random number 0 ≤ _r_ ≤ 1 (to choose if coalescence or speciation will happen).

9. Pick a random entry _V_<sub>i</sub> from the vector _V_ (to choose which lineage this will happen to).

10. If _r_ ≤ _θ_ / [_θ_+_K_− 1] , speciation happens: remove _V_<sub>i</sub> from the vector _V_ and append it to vector _W_ (then go to 12).

11. If _r_ > _θ_ / [_θ_ + _K_ − 1]  , coalescence happens: pick a random entry V<sub>j</sub> from vector _V_ (where _j_ not equal to _i_) and set _V_<sub>_i_</sub> = _V_<sub>_i_</sub> + _V_<sub>_j_</sub> then remove _V_<sub>_j_</sub> from the vector _V_.

12. Set _K_ = _K_ − 1; now the vector _V_ should have one less element.

13. If (_K_ > 0), we are not finished so go to 8.

14. If (_K_ ≤ 0), the algorithm is complete and _W_ contains the abundances of all species.
...
* Punchi-Manage, R. (2023h).  NetLogo Rosindell _et al._ (2010)-Algorithm-2.
http://netlogo/models/NetLogo-Rosindell-et-al-2010-Algorithm-2.
